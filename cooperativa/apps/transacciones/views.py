...
def depositar(request, numero):
    ...
    if usuario.groups.filter(name = "transacciones").exists():
        ...
        return render (request, 'transacciones/accion.html', locals())
    else:
        return render(request, 'login/forbidden.html', locals())
...
def verTransacciones(request,numero):
    cuenta = Cuenta.objects.get(numero=numero)
    listaTransacciones=Transaccion.objects.filter(cuenta=cuenta)
    return render(request, 'transacciones/index.html', locals())
